...
public class Camera2BasicFragment extends Fragment
    ...
    private void setUpCameraOutputs(int width, int height, int facingid) { //전면일 때
        Activity activity = getActivity();
        ...
        try {
            for (String cameraId : manager.getCameraIdList()) {
                ...
                if (facing != null && facing == facingid) { //0일때 전면이니, 만약 0과 같다면
     ...
     */
    private void openCamera(int width, int height) { //카메라 시작시
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.picture:
                ...
                break;
            case R.id.change: {
                if(facingid == CameraCharacteristics.LENS_FACING_FRONT){ //페이싱이 전면일 때
                    facingid = CameraCharacteristics.LENS_FACING_BACK; //클릭 시 후면이 될수있도록 함
                } else {
